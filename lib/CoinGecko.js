...
class CoinGecko {
    ...
    get coins() {
        ...
        return {
             ...
             * @param {string} coinId - (Required) The coin id (can be obtained from coins.list()) eg. bitcoin
             * @param {object} params - Parameters to pass through to the request
             * @param {string} params.vs_currency - (Required) The target currency of market data (usd, eur, jpy, etc.)
             * @param {integer} params.from - (Required) From date in UNIX Timestamp (eg. 1392577232)
             * @param {integer} params.to (Required) To date in UNIX Timestamp (eg. 1392577232)
             * @returns {ReturnObject}
             */
            fetchMarketChartRange: async(coinId, params={}) => {
                ...
                if (!Utils.isString(params['vs_currency']) || Utils.isStringEmpty(params['vs_currency'])) Utils._WARN_('Missing parameter', 'params must include `vs_currency` (eg. usd, eur, jpy, etc.)');

                if (!Number.isInteger(params['from'])) Utils._WARN_('Missing parameter', 'params must include `from`, date in UNIX Timestamp (eg. 1392577232)');

                if (!Number.isInteger(params['to'])) Utils._WARN_('Missing parameter', 'params must include `to`, date in UNIX Timestamp (eg. 1392577232)');
                const method = 'GET';
